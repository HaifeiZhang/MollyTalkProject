    ...
    public class LoginController : ControllerBase
    {
        ...
        public async Task<ActionResult> LoginByWXCode([FromBody] WXCodeRequest wXCode)
        {
            try
            {
                ...
                {
                    logger.LogError($"{code}生成OpenID失败");
                ...
                {
                    logger.LogError($"{openID}生成Token失败");
                    return BadRequest("Failed");
                }
            ...
            {
                logger.LogError($"{wXCode.Code}登录失败");
                logger.LogError($"{wXCode.Code}:{ex.Message}");   
                return Problem("登录失败");
            }
        ...
        public async Task<ActionResult> LoginByEmail([FromBody] EmailRequest request)
        {
            try
            {
                ...
                if (user == null)
                {
                    logger.LogError($"{request.EmailInfo}用户不存在");
                    return NotFound($"用户名不存在");
                }
                string jwtToken = await CheckUserAccountAsync(user, request.Pwd);
                if (jwtToken == null)
                {
                    logger.LogError($"{request.EmailInfo}生成Token失败");
                    return BadRequest("Failed");
                }
            ...
            {
                logger.LogError($"{request.EmailInfo}登录失败");
                logger.LogError($"{ex.Message}");
                return Problem($"{request.EmailInfo}登录失败");
            }
        ...
        private async Task<string> CheckUserAccountAsync(User user, string pwd)
        {
            try
            {
                ...
                if (!success)
                {
                    logger.LogError($"{user},{pwd}用户密码不正确！");
